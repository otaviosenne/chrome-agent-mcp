...
function formatChildren(node, nodeMap, depth) {
...
function formatNode(node, nodeMap, depth) {
    if (node.ignored)
        return "";
    const role = node.role?.value || "generic";
    if (SKIP_ROLES.has(role))
        return formatChildren(node, nodeMap, depth);
...
export function formatAccessibilityTree(nodes) {
    ...
    const root = nodes.find((n) => n.role?.value === "document" ||
        n.role?.value === "RootWebArea" ||
        n.role?.value === "WebArea");
    if (!root)
        return "Could not capture accessibility tree";
