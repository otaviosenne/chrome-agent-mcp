...
export const devtoolsElementsToolDefinition: Tool = {
...
export async function handleDevtoolsElements(
  args: Record<string, unknown>,
  connection: ChromeConnection
): Promise<ToolResult> {
  const client = await connection.getClient(args.tabId as string | undefined);
  const selector = args.selector as string;
  const includeStyles = (args.includeStyles as boolean | undefined) ?? false;
  ...
  if (includeStyles) {
    ...
    if (stylesResult.value) {
      const styles = JSON.parse(stylesResult.value as string);
      output += `\nComputed Styles:\n`;
